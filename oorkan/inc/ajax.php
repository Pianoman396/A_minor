...
if(!function_exists('qode_ajax_meta')) {
	...
	function qode_ajax_meta() {
        ...
        <div class="seo_title"><?phpphp wp_title(''); ?></div>
...
function get_quizzes(){
 $quiz_class = new Quiz();
    if(isset($_POST["step"]) && $_POST["step"]){
      ...
      $quizzes = $quiz_class->read([
      'post_type'   => 'quiz',
  		'posts_per_page' => -1, //-1 is all
      ...
      foreach ($quizzes as $quiz):
      	$content .= do_shortcode($quiz->post_content);
      endforeach;
      //var_dump($content);exit;
      echo $content;die();
	} else {
		echo "<section class='erorr'><code><b>Error:</b>empty data</code></section>";
	}

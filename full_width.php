...
const SliderConstructor = (options) => {
		...
		const getOffset = (dir) => {
			if (dir === 'center') return params.offsetAmounts.center;
			else if (dir === 'left') return params.offsetAmounts.left;
			else if(dir === 'right') return params.offsetAmounts.right;
		};
			...
			return new Promise((resolve, reject) => {
				...
				xhr.onreadystatechange = () => {
					if (xhr.readyState === 4 && xhr.status === 200) {
						...
						quizzes = xhr.responseText;
						// console.log(quizzes)
						return resolve(container.innerHTML = quizzes)
					}
				}
...
const sliderInit = (options) => {
		...
		const footerElements = () => {
			...
			<span id="quiz-count"> out of ${params.numberOfSlides} </span>
			</div>
		...
		.then(() => {
			horizontalSlider({
				numberOfSlides: '.oo-hz_slider>ul',
				slide: '.oo-hz_slider',
				slides:'.oo-hz_slider>ul',
				prevBtn:'.hz-btn-prev',
				nxtBtn: '.hz-btn-next',
	      autoStructure: false,
	      autoStart: false,
...
<script>
/*==================== HORIZONTAL SLIDER ====================*/
const horizontalSlider = (options) => {

      /*===== PARAMETERS =====*/
      let params = {
        container : document.querySelectorAll(options.container)[0] || document.getElementById('oo_slider_container'),
        numberOfSlides : document.querySelectorAll(options.numberOfSlides).length || document.getElementById('oo_slider_container').children,
        autoStructure : options.autoStructure || false,
        autoStart : options.autoStart || false,
        autoLoop : options.autoloop || false, // for disabling the buttons
        style : options.style || 'left',
        timer: options.timer || 5,
        activeElement : 0,
        offsetAmounts : {
      ...
      /*===== ELEMENTS =====*/
      let elements = {
        slider:  document.querySelectorAll(options.slider)[0] || document.getElementById('slider'),
        slides: document.querySelectorAll(options.slides) || document.querySelectorAll('.slides'),
        prevBtn: document.querySelectorAll(options.prevBtn)[0] || document.getElementById('btn-prev'),
        nxtBtn: document.querySelectorAll(options.nxtBtn)[0] || document.getElementById('btn-next'),
        firstSlide: document.getElementById('first_slide'),
        lastSlide: document.getElementById('last_slide')
      };
      ...
      const btnToggle = (slide, autoLoop) => {
        if (!autoLoop){
          if (slide <= 1) {
            disableBtn(elements.prevBtn)
            enableBtn(elements.nxtBtn)
          }else if(slide >= params.numberOfSlides - 2) { //3 -
            enableBtn(elements.prevBtn)
            disableBtn(elements.nxtBtn)
          }
        }
      ...
      const next = () => {
        ...
        if (params.activeElement + 1 < params.numberOfSlides) {
          ...
          moveSlide(params.activeElement, 'center=>left');
          params.activeElement++;
          enableBtn(elements.prevBtn) // correct
        } else {
          moveSlide(0, 'right=>center');
          moveSlide(params.activeElement, 'center=>left');
          params.activeElement = 0;
